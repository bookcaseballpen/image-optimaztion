...
IF EXIST %1%/ (
    ...
    cd %1
    md optmz
    for %%i  in (*.jpg) do "C:\Image Optimization\cjpeg.exe" -quality 75 "%%i" > "%%~pi\optmz\%%~ni%%~xi" 
    for %%i in (*.png) do "C:\Image Optimization\pngquant.exe" --force --verbose 256 "%%i" -o "%%~pi\optmz\%%~ni%%~xi"
